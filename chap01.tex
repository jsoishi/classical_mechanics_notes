...
\begin{equation}
  ...
  \end{split}
\end{equation}
where here we note that in the expression $\var{J[y]}$, $\delta$ is an \textbf{operator}, but in $\delta y(x)$, it's just the name of the function. Now, we make use of the chain rule\marginnote{The chain rule is something you should know cold. It's given by $\dv{f(g(x))}{x} = \dv{f}{g} \dv{g}{x}$} as applied to a multivarible function, $F(x, y(x), y'(x))$:
...
\end{equation}
We want to integrate the second term by parts because it's in the canonical form, and because we know that the boundary term will be zero.\marginnote{Integration by parts is also something you should totally know. It's given by $\int_a^b f(x) g'(x) dx = \eval{f(x)g(x)}_{x = a}^{x=b} - \int_a^b f'(x) g(x) dx.$ The first term is called the ``boundary term.''} This gives 
\begin{equation}
  \label{eq:first_order_functional_step2}
